...
func TestTimingWheel_AfterFunc(t *testing.T) {
	...
	for _, d := range durations {
		t.Run("", func(t *testing.T) {
			...
			minV := start.Add(d).Truncate(time.Millisecond)

			err := 5 * time.Millisecond
			if got.Before(minV) || got.After(minV.Add(err)) {
				t.Errorf("Timer(%s) expiration: want [%s, %s], got %s", d, minV, minV.Add(err), got)
			}
		})
	}
}
...
func TestTimingWheel_ScheduleFunc(t *testing.T) {
	...
	for _, d := range s.intervals {
		got := (<-exitC).Truncate(time.Millisecond)
		accum += d
		minV := start.Add(accum).Truncate(time.Millisecond)

		err := 5 * time.Millisecond
		if got.Before(minV) || got.After(minV.Add(err)) {
			t.Errorf("Timer(%s) expiration: want [%s, %s], got %s", accum, minV, minV.Add(err), got)
		}
	}
}
